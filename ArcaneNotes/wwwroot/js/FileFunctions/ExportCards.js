...
export function GetCurrentNoteData() {
    ...
    const defaultTitle = document.getElementById("defaultNoteTitle");
    const defaultGmName = document.getElementById("defaultGmName");
    const noteType = document.getElementById("noteType");

    return {
        Title: titleInput?.value?.trim() || defaultTitle?.value || "",
        Name: gmNameInput?.value?.trim() || defaultGmName?.value || "",
        Type: noteType?.value || "gm",
        NoteData: allCards
    };
}
...
function getCardData(containerElement) {
    ...
    const cards = [];
    const cardContainers = containerElement.querySelectorAll(":scope > .card-container");
    ...
    cardContainers.forEach((container) => {
        ...
        if (cardType === "Group_Card") {
            ...
            const exportFn = cardFunctions[cardType];

            if (typeof exportFn === "function") {
                cardObject.CardData = exportFn(container);
            }
